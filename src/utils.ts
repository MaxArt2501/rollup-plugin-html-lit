...
export const getExpressionPositions = (html: string): Interval[] => {
  ...
  return expression.kind === SyntaxKind.TemplateExpression
    ? (expression as TemplateExpression).templateSpans.flatMap((span) =>
        getAllChunks(span.expression)
      )
    : [];
};
 ...
 */
export const isChunk = (value: any): value is ScriptChunk =>
  typeof value?.pos === "number" && typeof value!.end === "number";
...
export const getAllChunks = <T extends ScriptChunk>(chunk: T): Interval[] => {
  ...
  const intervals: Interval[] = [
    ...positions,
    ...getUncoveredIntervals([chunk.pos, chunk.end], positions),
 ...
 *
 */
export function* getUncoveredIntervals(
  interval: Interval,
  chunks: Array<Interval>
) {
  const [start, end] = interval;
  let index = start;
  while (index < end) {
    const containingChunk = chunks.find(
      ([chunkStart, chunkEnd]) => index >= chunkStart && index < chunkEnd
    );
    if (containingChunk) {
      index = containingChunk[1];
    } else {
      const nextChunk = chunks.reduce(
        (prevChunk: Interval | null, curChunk: Interval) => {
          if (
            curChunk[0] > index &&
            (!prevChunk || prevChunk[0] > curChunk[0])
          ) {
            return curChunk;
          }
          return prevChunk;
 ...
 */
export const generateHTMLSourceMap = (
  html: string,
  minified: string,
  transformed: string
) => {
  const rawBits = getExpressionPositions(html);
  const rawUncov = Array.from(
    getUncoveredIntervals([0, html.length + 1], rawBits)
  );
  const minBits = getExpressionPositions(minified);
  const minUncov = Array.from(
    getUncoveredIntervals([0, minified.length + 1], minBits)
  );
  const headerShift = transformed.indexOf(minified);

  const allRawChunks = [...rawBits, ...rawUncov]
    .sort(([start1], [start2]) => start1 - start2)
    .map(([start]) => getCoords(html, start - 1));
  const allMinChunks = [...minBits, ...minUncov]
    .sort(([start1], [start2]) => start1 - start2)
    .map(([start]) => getCoords(transformed, start + headerShift - 1));

  const minRowChunks = Array.from(
    { length: allMinChunks[allMinChunks.length - 1][0] + 1 },
    (_, row) =>
      allMinChunks
        .filter(([chRow]) => chRow === row)
        .map(([, column]) => column)
  );
...
const ARGS_RE =
  /^\s*<!--\s*@arguments\s+\(\s*([a-z_]\w*(?:\s*,\s*[a-z_]\w*)*)\s*\)\s*-->/im;
